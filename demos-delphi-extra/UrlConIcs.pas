...
{ *                                                       * }
{ * April 2019 - Angus fixed authentication added NoCache * }
{ *                                                       * }
{ * May 2022 - V8.69 simplified authentication to use     * }
{ *            WWWAuthInfos with multiple methods         * }
{ Dec 18, 2023 V9.1  Added OverbyteIcsSslBase which now includes
{                    TSslContext,TX509Base and TX509List.   }
{ Aug 07, 2024 V9.3  Added OverbyteIcsTypes for consolidated
                     types and constants, allowing
...
//        FBasicAuth       : Boolean;   ANGUS need to handle more types
        FServerAuth      : THttpAuthType;  // ANGUS gone V8.69
...
//        property BasicAuth : Boolean read FBasicAuth write FBasicAuth;
        ...
        property WWWAuthInfos : TWWWAuthInfos  read FWWWAuthInfos;        { V8.69  }
    end;
...
//    HTTP.ProxyPort        := FProxyPort;
    ...
    HTTP.ResponseNoException := True;   { V8.69 }
end;
...
begin
    ...
    FWWWAuthInfos       := HTTP.WWWAuthInfos;   { V8.69 array with authorisation types and releams }
    FServerAuth := httpAuthNone;
  (*  if HTTP.AuthorizationRequest.Count > 0 then { AG }  not needed, info now in WWWAuthInfos
    begin
