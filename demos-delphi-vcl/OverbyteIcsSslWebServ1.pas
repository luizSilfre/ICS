...
July 4, 2023 V8.71 Added localhost.pem as default certificate.
                   OCSP check works again.
                   Added 'Request Client Cert' to Verify Peer tick box since this is what it does.
Aug 08, 2023 V9.0  Updated version to major release 9.
Feb 08, 2023 V9.1  Added OverbyteIcsSslBase which now includes TSslContext,TX509Base and TX509List.
                   ...
                   New certificate and wwwroot locations.
Sep 16, 2024  V9.3 Using OverbyteIcsTypes for consolidated types and constants, allowing
                     other import units to be removed.
                   Fixed HHTPS typo.
                   Renegotiation gone, no longer supported.
                   Added cipher list for TLSv13, crypto groups list, updated cipher lists
                     for TLSv12.
...
uses
  ...
  OverbyteIcsIniFiles, StdCtrls, ExtCtrls, TypInfo,
//  OverbyteIcsWinsock,
  OverbyteIcsWSocket,
  OverbyteIcsWSocketS,
  OverbyteIcsHttpSrv,
...
const
  CopyRight : String         = 'WebServ (c) 1999-2024 F. Piette V9.3 ';
...
type
  ...
  TSslWebServForm = class(TForm)
    ...
    SslCipher13Edit: TEdit;
    Label13: TLabel;
    Label19: TLabel;
    SslCryptoGroups: TEdit;
...
//  KeyECDHList        = 'ECDHList';
    ...
    KeySslCipher13     = 'SslCipher13';
    KeySslCryptoGroups = 'SslCryptoGroups';
...
begin
    ...
    { Restore persistent data from INI file }
        ...
        DocDirEdit.Text     := IniFile.ReadString(SectionData, KeyDocDir, '');             { V9.1 }
        DefaultDocEdit.Text := IniFile.ReadString(SectionData, KeyDefaultDoc, 'index.html');
        PortHttpsEdit.Text  := IniFile.ReadString(SectionData, KeyPortHttps, '443');
        PortHttpEdit.Text   := IniFile.ReadString(SectionData, KeyPortHttp, '80');
        DisplayHeaderCheckBox.Checked := Boolean(IniFile.ReadInteger(SectionData, KeyDisplayHeader, 0));
        WriteLogFileCheckBox.Checked := Boolean(IniFile.ReadInteger(SectionData, KeyLogToFile, 0));
        DisplaySslInfoCheckBox.Checked := Boolean(IniFile.ReadInteger(SectionData, KeyDisplaySslInfo, 0));
        CertFileEdit.Text    := IniFile.ReadString(SectionData, KeyCertFile, GSSL_CERTS_DIR + GSSL_LOCALHOST_NAME);  { V9.1 new location}
        ...
        CAFileEdit.Text      := IniFile.ReadString(SectionData, KeyCAFile, '');      { V9.1 not needed }
        CAPathEdit.Text      := IniFile.ReadString(SectionData, KeyCAPath, '');
        AcceptableHostsEdit.Text := IniFile.ReadString(SectionData, KeyAcceptableHosts, '');      { V9.1 not needed }
        VerifyPeerCheckBox.Checked := Boolean(IniFile.ReadInteger(SectionData,  KeyVerifyPeer,  0));
        FRenegotiationInterval := IniFile.ReadInteger(SectionData,   KeyRenegInterval, 0);
        SslMinVersion.ItemIndex := IniFile.ReadInteger(SectionData, KeySslMinVersion,  0);
        SslMaxVersion.ItemIndex := IniFile.ReadInteger(SectionData, KeySslMaxVersion, 5);
        SslCipherList.ItemIndex := IniFile.ReadInteger(SectionData, KeySslCipherList, 2);    { V8.05 }
        ...
        SslCipher13Edit.Text := IniFile.ReadString(SectionData, KeySslCipher13, sslCipherSuitesTLS13);     { V9.3 }
        SslCryptoGroups.Text := IniFile.ReadString(SectionData, KeySslCryptoGroups, sslCryptoGroupsDef);   { V9.3 }
        IniFile.Free;

    { V9.1 new sample directory structure }
      ...
      { V8.05 allow user to choose which IP address to listen }
       ...
       { V8.52 get local IP list from newer cross platform function }
         ...
         //   InterfaceList.AddStrings(LocalIPList(sfAny));   { V8.52 show IPv6 as well }
            ListenAddr.Items.Text := 'localhost' + #13#10 + '0.0.0.0' + #13#10 + InterfaceList.Text;
        finally
            InterfaceList.Free;
        end;
        ...
        Display('    Winsock:');
        Display('        Version ' + Format('%d.%d', [WinsockInfo.wHighVersion shr 8,  WinsockInfo.wHighVersion and 15]));
   ...
   //     GSSLEAY_DLL_IgnoreNew := OldSslCheckBox.Checked;  { V8.66 ignore OpenSSL 3.0 and later }
     //   SslStaticLock1.Enabled := true ;  { V8.66 gone with 1.0.2, note also loaded OpenSSL }
      //  LoadSsl;                            { V8.66 need version number }
        ...
        DisplayMemo.Lines.Add('SSL/TLS DLL: ' + IcsReportOpenSSLVer(True));    { V9.1 simplify }
...
begin
    ...
    IniFile.WriteInteger(SectionData,   KeyLogToFile,  ord(WriteLogFileCheckBox.Checked));
    IniFile.WriteInteger(SectionData,   KeyDisplaySslInfo,  ord(DisplaySslInfoCheckBox.Checked));
    ...
    IniFile.WriteString(SectionData,    KeyDocDir,  DocDirEdit.Text);                  { V9.1 }
    IniFile.WriteString(SectionData,    KeySslCipher13, SslCipher13Edit.Text);         { V9.3 }
    IniFile.WriteString(SectionData,    KeySslCryptoGroups, SslCryptoGroups.Text);     { V9.3 }
    IniFile.UpdateFile;
    IniFile.Free;
    CloseLogFile;
end;
...
var
//    CAList: TX509List;       { V8.64 gone V9.1 }
    ErrStr: string;
    valres: TChainResult;
//    FDir, FName: String;
    Inters: TX509List;
begin
    if NOT FileExists(CertFileEdit.Text) then begin
        Display('HTTPS Not Started, Can Not Find Server SSL/TLS Certificate: ' + CertFileEdit.Text);
        Exit;
    end;
...
//  IcsLogger1.LogOptions := IcsLogger1.LogOptions + LogAllOptDump ; { SSL devel dump }
  ...
  { V8.41 new way, supports PEM, DER, PKCS12, PKCS8 formats and check them earlier }
    ...
    if (NOT SslContext1.SslCertX509.IsPkeyLoaded) and (PrivKeyFileEdit.Text <> '') then   { V8.71 need file }
            SslContext1.SslCertX509.PrivateKeyLoadFromPemFile(PrivKeyFileEdit.Text);
    if (NOT SslContext1.SslCertX509.IsInterLoaded) and (CAFileEdit.Text <> '') then       { V8.71 need file }
 ...
 //   CAList.LoadAllFromString(sslRootCACertsBundle);  { V8.64 trusted root so we check chain }
    valres := SslContext1.SslCertX509.ValidateCertChain('', IcsSslRootCAStore, FSrvSslCert, ErrStr);  { really need host name, V9.1 new store  }
    if ErrStr <> '' then
       ErrStr := ', ' + ErrStr;
  ...
  { V8.69 check OCSP server if revoked and keep OCSP response for status stapling }
        ...
        if (SslContext1.SslCertX509.UrlOcsp <> '') then begin
           ...
           //     CAList.SetX509Store;
             ...
             // must block and wait for OSCP response
                if OcspSrvHttp.CheckOcspRevoked(IcsSslRootCAStore.X509Store, 3) then begin   // three seconds wait V8.71 pass correct pointer
                    valres := chainFail;
                end;
    ...
    Display('HTTPS Not Started: ' + FSrvSslCert + #13#10 + ErrStr + #13#10);  { V9.3 fixed typo }
    if valres = chainFail then
        Exit;
    ...
    case SslCipherList.ItemIndex of   { V8.05 choice of ciphers, V9.3 two old ciphers gone, high now v12 }
        0: SslContext1.SslCipherList := sslCiphersServer;
        1: SslContext1.SslCipherList := sslCiphersMozillaSrvBack;
        2: SslContext1.SslCipherList := sslCiphersMozillaSrvTLS12;
    end;
   if SslCipherEdit.Text <> '' then
        SslContext1.SslCipherList := SslCipherEdit.Text; { V8.05 }
   if SslCipher13Edit.Text <> '' then
        SslContext1.SslCipherList13 := SslCipher13Edit.Text     { V9.3 }
   else
        SslContext1.SslCipherList13 := sslCipherSuitesTLS13;    { V9.3 }
   SslContext1.SslCryptoGroups := SslCryptoGroups.Text;         { V9.3 }
...
begin
    Body := '<HTML>' +
          ...
          '<BODY>' +
            ...
            '<A HREF="https://www.overbyte.be">ICS Home page</A><P>' + #13#10 +
            '<P>OpenSSL Version: ' + IcsReportOpenSSLVer + '<BR>' + #13#10#13#10 +     { V9.1 }
             ...
             'Note: There is a better demo OverbyteIcsSslMultiWebServ.<P>' + #13#10 +
          '</BODY>' + #13#10 +
        '</HTML>' + #13#10;
