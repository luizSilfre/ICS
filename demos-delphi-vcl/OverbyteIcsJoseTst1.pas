...
// not saved
    ...
    procedure TestJWSAlgChange(Sender: TObject);
  private
...
procedure TJsonDemoForm.TestJWSAlgChange(Sender: TObject);     { V9.4 clear private key ready for new key }
begin
     FPrivateKey.ClearAll;
     LabelPKey.Caption := 'Private Key: ';
end;
...
begin
    ...
    if TestJWSAlg.ItemIndex > Ord(jsigEdDSA) then begin
    ...
    try
        if NOT Assigned(FPrivateKey.PrivateKey) then begin
            ...
            if PKeyType = EVP_PKEY_RSA then
                FPrivateKey.PrivKeyType := PrivKeyRsa2048
            else if PKeyType = EVP_PKEY_EC then
                FPrivateKey.PrivKeyType := PrivKeyECsecp256
            else if PKeyType = EVP_PKEY_RSA_PSS then
                FPrivateKey.PrivKeyType := PrivKeyRsaPss2048
            else if PKeyType = EVP_PKEY_X25519 then
                FPrivateKey.PrivKeyType := PrivKeyEd25519;
            LabelPKey.Caption := 'Private Key: ';
            AddLog('Generating private and public key pair, please wait');
            FPrivateKey.DoKeyPair;
            AddLog('Generated private and public key pair OK: ' + FPrivateKey.PrivateKeyInfo);
        ...
        NewKeyType := IcsPkeyid(FPrivateKey.PrivateKey);
        BaseType := IcsPkeyBaseid(FPrivateKey.PrivateKey);
        if (PKeyType <> BaseType) and  (NewKeyType <> EVP_PKEY_ED25519) then begin
            AddLog ('Mismatch Private Key Type and Signing Hash Algorithm');
            Exit;
        end;
        AddLog ('Data: ' + JoseTextLines.Lines.Text + IcsCRLF +
                'Private Key: ' + fPrivateKey.PrivateKeyInfo + IcsCRLF +
                'Digest Type: ' + EvpDigestLits[SignDigest] );

        SigTB := IcsAsymSignDigestTB(DataTB, fPrivateKey.PrivateKey, SignDigest);
        AddLog ('Digess Length (binary): ' + IntToStr(Length(SigTB)) + ' bytes');
  ...
  //      TestSignDigest(AnsiData, TestPrivKeyFile.Text, 'sha256');
        ...
        Flag := IcsAsymVerifyDigestTB(DataTB, SigTB, FVerifyPKey.PrivateKey, SignDigest);
        S := 'Failed Verify';
...
begin
   ...
   // build Json Web Key
    ...
    // build JWS in compacyt (x.x.x) and Json formats
        ...
        JsonJWSKey := IcsJoseJWSJson(JoseAlg, Data, TestHmacKey.Text, FPrivateKey.PrivateKey, 'jws', KwkPub, '', '103');
        RawJWK.Text := JsonJWSKey;
        RespJson := SO(JsonJWSKey);
        AddLog ('IcsJoseJWS Json JWK: ' + RespJson.AsJson(true, false));

        JsonJWSKid := IcsJoseJWSJson(JoseAlg, Data, TestHmacKey.Text, FPrivateKey.PrivateKey, 'jws', '', MyKid, '104');
