...
Oct 20, 2022  V8.70 Added NoRecvStreamCheckBox to test receiving data in DocData instead
                of RecvStream, specifically for content encoding.
              Display HTML content as UTF-8 decoding entities.
...
Note - OverbyteIcsHttpCCodZlib is not used directly by this project, but causes
Content Encoding support to be enabled.
...
const
  HttpTstVersion         = 903;
  CopyRight : String     = 'HttpTst (c) 1997-2024 Francois Piette  V9.3 ';
...
type
  THttpTestForm = class(TForm)
    ...
    NoRecvStreamCheckBox: TCheckBox;
    ...
    procedure HttpCli1SelectDns(Sender: TObject; DnsList: TStrings;
      var NewDns: string);
    procedure HttpCli1DocData(Sender: TObject; Buffer: Pointer; Len: Integer);
  private
    Initialized  : Boolean;
    DocFileName  : String;
    FIniFileName : String;
...
begin
    if not Initialized then begin
        ...
        NoRecvStreamCheckBox.Checked := Boolean(IniFile.ReadInteger(SectionData, KeyNoRecvStream, 0));      {V8.70 }
...
begin
    ...
    IniFile.WriteInteger(SectionData,   KeyNoRecvStream, Ord(NoRecvStreamCheckBox.Checked));     {V8.70 }
{$IFDEF UseNTLMAuthentication}
...
{$ENDIF}
        ...
        if Length(DocFileName) = 0 then begin
            if NOT NoRecvStreamCheckBox.Checked then   { V8.70 }
...
//                    DocumentMemo.Lines.LoadFromStream(DataIn)
                  { V8.70 convert to unicode, with entities }
                    DocumentMemo.Lines.Text := String(IcsHtmlToStr(DataIn, HttpCli1.ContentType, True))
                else begin
...
{$ENDIF}
        ...
        if HttpCli1.ContentLength = 0 then begin
            ...
            try
                if Copy(HttpCli1.ContentType, 1, 5) = 'text/' then
                    ...
                    DocumentMemo.Lines.Add('Content type is ' +  HttpCli1.ContentType);
                    DocumentMemo.Lines.Add('Document stored in ''' + DocFileName + ''' Size=' + IntToStr(DataIn.Size));
                end;
            finally
                DataIn.Free;
            end;
...
begin
    ...
    Display('Content Length = ' + IntToStr(HttpCli1.ContentLength)); { V8.70 }

    if NoRecvStreamCheckBox.Checked then Exit;  { V8.70 }
...
procedure THttpTestForm.HttpCli1DocData(Sender: TObject; Buffer: Pointer; Len: Integer);      { V8.70 }
var
    HtmlData: TBytes;
begin
    Display('DocData Event, Data Length = ' + IntToStr(Len));
    if NOT NoRecvStreamCheckBox.Checked then Exit;
    if Pos('text', HttpCli1.ContentType) > 0 then begin
        SetLength(HtmlData, Len);
        Move(Buffer^, HtmlData[0], Len);
     // may come here several times
        DocumentMemo.Lines.Append(String(IcsHtmlToStr(HtmlData, Len, CP_UTF8, True)));
    end;
end;
