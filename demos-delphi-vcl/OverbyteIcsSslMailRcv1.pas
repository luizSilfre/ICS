...
Dec 19, 2022 V8.71 Support OAuth authentication OAuthTypeEmbed (embedded browser)
                ...
                uses TEdgeBrowser or TWebBrowser to display web page without needing
...
//OverbyteIcsSslHttpRest,
  ...
  OverbyteIcsSslBase,  { V8.69 }
  OverbyteIcsTypes, OverbyteIcsWndControl;  { V9.3 consolidated types and constants }

const
    MailRcvVersion = 903;
    CopyRight : String = ' MailRcv demo (c) 1997-2024 F. Piette V9.3 ';
...
begin
    ...
    try
        ...
        PassWordEdit.Text      := IniFile.ReadString(SectionData, KeyPassword,
                                                     '');
        AuthComboBox.ItemIndex := IniFile.ReadInteger(SectionData, KeyAuth, Ord(popAuthLogin));       { V8.65 SSL defaults }
        SslTypeComboBox.ItemIndex := IniFile.ReadInteger(SectionSSL, KeySslType, Ord(pop3TlsImplicit));
...
//  IcsRestEmail1.ClientSecret := 'xxxxx';
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TPOP3ExcercizerForm.IcsRestEmail1OAEmbed(Sender: TObject; const URL: string; var Success: Boolean);  { V8.71 }
begin
