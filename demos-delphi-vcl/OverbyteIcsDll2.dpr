...
Dec 12, 2024 V9.4  Supporting SSL by replacing THttpCli with TSslHttpRest.
...
const
  IcsDll2Version            = 904;
  CopyRight    : String     = ' IcsDll2 (c) 2024 Francois Piette V9.4 ';
...
type
  ...
  public
    constructor Create;
    property HttpCli       : TSslHttpRest  read FHttpCli      write FHttpCli;
...
begin
    ...
    try
        ...
        FHttpCli                    := TSslHttpRest.Create(nil);
        try
//          FHttpCli.Url                := String(StrPas(FUrl));
            FHttpCli.OnRequestDone      := HttpCliRequestDone;
//          FHttpCli.RcvdStream         := TMemoryStream.Create;
//          FHttpCli.GetAsync;
            FHttpCli.RestRequest(httpGET, String(Url), True, '');  // third arg is false for sync, fourth is raw params
...
begin
    if ErrCode <> 0 then begin
        ...
        // Success
        FErrorCode^ := FHttpCli.StatusCode;
        N := Length(FHttpCli.ResponseOctet);
        if N > FBufSize^ then begin
            // Supplied buffer is too small
            N         := FBufSize^;                 // Truncate length to copy
            FBufSize^ := -Length(FHttpCli.ResponseOctet);  // Return negative size
        end
        else
            FBufSize^ := N;                         // Return document size
        // Copy data to buffer
        Move(FHttpCli.ResponseOctet[1], Buffer^, N);
    end;
