...
Aug 10, 2023  V8.71 removed another warning.
Aug 08, 2023 V9.0  Updated version to major release 9.
Feb 12, 2024 V9.1  mime.type and test files now in demos-data directory.
                   Segio font so scales properly.
May 14, 2024 V9.2  Added button to allow decode file to be selected.
                   Added 'Loose RFC Rules' tick box to allow decoding MIME parts
                     that don't meet RFC requirements, as some other apps do.
                   Enlarged windows, longer edit fields.
                   Reported part level for embedded parts, using PInfo for
                     display instead of building line here.
Jul 06, 2024 V9.3  Added Test HTTP Headers to test IcsPaseHttpHdr function.
...
{$ENDIF}
  ...
  OverbyteIcsHtmlUtils;          { V9.3 }

const
  MimeDemoVersion    = 904;
  CopyRight : String = ' MimeDemo (c) 1998-2024 F. Piette V9.4 ';
...
{$ENDIF}
    ...
    procedure doTestHttpHdrsClick(Sender: TObject);
  private
    Memo1          : TCurrentMemo;
...
const
    ...
    KeyFile       = 'FileName';
    KeyText       = 'TextEdit';
    KeyIgnore     = 'IgnoreBlankParts';
    KeyLooseRFC   = 'LooseRFC';
...
begin
    if not FInitialized then begin
        ...
        IgnoreBlankParts.Checked := IniFile.ReadBool(SectionData,  KeyIgnore, IgnoreBlankParts.Checked); { V9.2 }
        LooseRFC.Checked := IniFile.ReadBool(SectionData,  KeyLooseRFC, LooseRFC.Checked);     { V9.2 }
        IniFile.Free;
...
begin
    ...
    IniFile.WriteBool(SectionData,  KeyIgnore,   IgnoreBlankParts.Checked);  { V9.2 }
    IniFile.WriteBool(SectionData,  KeyLooseRFC, LooseRFC.Checked);     { V9.2 }
    IniFile.UpdateFile;
    IniFile.Free;
end;
...
begin
    Memo1.Clear;
    Update;
    MimeDecode1.LooseRFC := LooseRFC.Checked;   { V9.2 }
...
begin
    Display('--------- PART ' + IntToStr(MimeDecode1.PartNumber) + ' BEGIN ----------');
    FCharCnt := 0;
end;
...
begin
    ...
    Display('--------- PART ' + IntToStr(MimeDecode1.PartNumber) + ' END ----------');
    { Close file, if any }
...
begin
    Display(#13#10'--- Some Msg header: lines MIME inline decoded to Unicode ---' + #13#10 +
...
begin
    Display('--------- PART ' + IntToStr(MimeDecode1.PartNumber) + ' HEADER BEGIN ----------');
    if MimeDecode1.PartLevel > 0 then
        Display('--------- EMBEDDED PART LEVEL ' + IntToStr(MimeDecode1.PartLevel) + ' ---------');   { V9.2 }
end;
...
begin
    Display('--------- PART ' + IntToStr(MimeDecode1.PartNumber) + ' HEADER END ----------');
end;
...
var
    ...
    Txt : String;
begin
    Buf := String(IcsBase64Decode(TextEdit.Text));
    TextEdit.Text := Buf;
    Txt := '';
...
var
    Buf : String;
begin
    Buf := IcsBase64Encode(TextEdit.Text);
    TextEdit.Text := Buf;
    Memo1.Lines.Add(Buf);
end;
...
begin
    ...
    Display('Unicode Header: ' + UniStr + ' [CharSet=' + String(CharSet) + ']');
    Display('');
end;
...
begin
    ...
    Display('UTF-8 Text: ' + String(AnsiString(StringToUtf8 (TextEdit.Text))));    { V8.70 was string, V8.71 fix warning }
    Display('UTF-8 Binary: ' + String(HdrEncodeInLineEx(TextEdit.Text, SpecialsRFC822, 'B', CP_UTF8 , 72, false)));
    Display('UTF-8 Quoted: ' + String(HdrEncodeInLineEx(TextEdit.Text, SpecialsRFC822, 'Q', CP_UTF8 , 72, false)));
    Display('ISO-8859-1 Binary: ' + String(HdrEncodeInLineEx(TextEdit.Text, SpecialsRFC822, 'B', 28591, 72, false)));
    Display('ISO-8859-1 Quoted: ' + String(HdrEncodeInLineEx(TextEdit.Text, SpecialsRFC822, 'Q', 28591, 72, false)));
    Display('Current Binary: ' + String(HdrEncodeInLineEx(TextEdit.Text, SpecialsRFC822, 'B', CP_ACP, 72, false)));
    Display('Current Quoted: ' + String(HdrEncodeInLineEx(TextEdit.Text, SpecialsRFC822, 'Q', CP_ACP, 72, false)));
end;
...
begin
    ...
    MimeDecodeEx1.LooseRFC := LooseRFC.Checked;   { V9.2 }
   ...
   //       Display(DecodeMimeInlineValue (UnfoldHdrValue (MimeDecodeEx1.HeaderLines [I])));
    ...
    if MimeDecodeEx1.DecParts = 0 then
        ...
        for I := 0 to Pred (MimeDecodeEx1.DecParts) do begin
            Display(MimeDecodeEx1.PartInfos [I].Pinfo);  { V9.2 easy to display part information }
...
begin
    ...
    begin
        Display ('Mime Types List Loaded from Windows Registry, took ' + IntToStr (GetTickCount - tickstart) + 'ms');
...
begin
    tickstart := GetTickCount;
    MimeTypesList1.MimeTypesFile := '..\..\..\..\demos-data\mime.types';   { V9.1 new directory }
...
const
    ...
    begin
        tickstart := GetTickCount;
        for I := 1 to tests do
            S := MimeTypesList1.TypeFromExtn (Extn);
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
{ few examples of HTTP request and response headers, mostly with multiple arguments that
  we parse into their parts }

const
    TotHttpHdrs = 39 ;
var
    TestHttpHdrs: array [1..TotHttpHdrs] of String = (
        'Accept: text/html',
        'Accept: image/gif, image/x-xbitmap, image/jpeg, image/pjpeg, */*',
        'Accept: text/html, application/xhtml+xml, application/xml;q=0.9, */*;q=0.8',    // 3
        'Accept-Encoding: gzip, deflate',
        'Allow: GET, POST, HEAD',
        'Content-Security-Policy: default-src ''self'' http://example1.com; connect-src ''none'';',    // 6
        'Content-Security-Policy: connect-src http://example1.com/;' + IcsCRLF + 'script-src http://example2.com/',    { probably illegal }
        'Content-Disposition: form-data; name="myFile"; filename="foo.txt"',
        'Content-Type: text/plain',
        'Content-Type: text/html; charset="iso-8859-1"',     // 10
        'Content-Type: text/html; charset=''utf-8''',        // 11
        'Content-Type: json/xml; charset=utf-8',             // 12
        ...
        'Content-Type: application/json;charset=UTF-8;odata.metadata=minimal',    // 25
        'Cookie: name=value',
        'Cookie: name=value; name2=value2; name3=value3',
        'Keep-Alive: timeout=5, max=1000',
        'Set-Cookie: WebAppTelecom-Logon=_NONE_; EXPIRES=Sat, 29 Jun 2024 00:00:00; DOMAIN=telecom-tariffs.co.uk; PATH=/; Secure; HttpOnly',  // 29
        'Strict-Transport-Security: max-age=31536000; includeSubDomains',
        'X-Content-Type-Options: nosniff',
        'X-XSS-Protection: 1;mode=block',
        'Authorization: Basic YW5ndXM3OmZnaA==',                                                                                              // 33
        'Authorization: Digest username="Mufasa",realm="testrealm@host.com",nonce="dcd98c093",uri="/dir/index.html",qop=auth,nc=00000001,cnonce="0a4f113b",response="6629f07c4ef1", opaque="5cc71e9517f40e41"',   // 34
        'Authorization: Digest username="Mufasa",realm="http-auth@example.org",uri="/dir/index.html", algorithm=SHA-256,nonce="7ypf/xlj90v",nc=00000001,cnonce="f2/wEemxURZJ", qop=auth, response="8c00eebdbec",opaque="FQhe/qtdS"',
        'WWW-Authenticate: Basic Realm="Business Telecom Tariffs (all lower case please)"',                                                                                                                      // 36
        ...
        'WWW-Authenticate: Basic Realm="DemoAuthAll"'                    // 39
        );


{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TMimeDecodeForm.doTestHttpHdrsClick(Sender: TObject);
var
    I: Integer;

    procedure Parse1(const Hdr, Sep: String);
    var
        HdrName: String;
        HdrArgs: THdrArgs;
        J, Tot: Integer;
    begin
        Display('Parsing: ' + Hdr);
        Tot := IcsPaseHttpHdr(Hdr, HdrName, HdrArgs);
        if Tot = 0 then
            Display('No arguments found, header name: ' + HdrName)
        else begin
            Display(InttoStr(Tot) +  ' arguments found, header name: ' + HdrName);
            for J := 0 to Tot - 1 do
                Display('Arg ' + InttoStr(J + 1) + ': name: ' + HdrArgs[J].HName + ': value: ' + HdrArgs[J].HValue);
        end;
        Display('');
    end;

begin
 // parse all with automatic separator
    for I := 1 to TotHttpHdrs do begin
        Parse1(TestHttpHdrs[I], '');
    end;

    Display('Repeating some with known separators');
    Parse1(TestHttpHdrs[3], ',');
    Parse1(TestHttpHdrs[4], ';');
    Parse1(TestHttpHdrs[6], ' ');
    Parse1(TestHttpHdrs[29], '=');  // set-cookie
    Parse1(TestHttpHdrs[33], ';');
    Parse1(TestHttpHdrs[34], ';');
    Parse1(TestHttpHdrs[36], ';');
    Parse1(TestHttpHdrs[39], '=');

    Display('Finding charset and MIME from Content-Type headers');
    Display(IcsFindHdrArg(TestHttpHdrs[10], 'charset'));
    Display(IcsFindHdrArg(TestHttpHdrs[10], ''));      // mime
    Display(IcsFindHdrArg(TestHttpHdrs[11], 'charset'));
    Display(IcsFindHdrArg(TestHttpHdrs[11], ''));
    Display(IcsFindHdrArg(TestHttpHdrs[12], 'charset'));
    Display(IcsFindHdrArg(TestHttpHdrs[12], ''));
    Display(IcsFindHdrArg(TestHttpHdrs[25], 'charset'));
    Display(IcsFindHdrArg(TestHttpHdrs[25], ''));

end;

{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TMimeDecodeForm.SelectFileClick(Sender: TObject);   { V9.2 }
begin
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TMimeDecodeForm.CloseButtonClick(Sender: TObject);
begin
    Close;
end;
