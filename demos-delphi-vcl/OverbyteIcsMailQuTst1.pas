...
25 Oct 2022 - V8.70 - Set SMPT UTF-8 charset so headers get inline MIME
                 encoded if necessary, rather than being converted to ANSI.
                 Note body lines are not converted in this sample.
              Fixed memory leaks.
              Added Rest Email Microsoft User Authority property to access
                different user authorities, defaults to consumers but can be
                changed to common or an Azure Active Directory tenant GUID for
                corporate accounts.
10 Apr 2023 - V8.71 Support OAuth authentication OAuthTypeEmbed (embedded browser) using
              ...
              Set IcsRestEmail account hint to login name for browser hint.
              MX DNS lookup now uses simpler sync method and handles alternate
...
May 17, 2024 V9.2  Added 'Send Prepared EML File' to queue an existing EML file rather then
                     preparing email with properties.
...
Note, this sample only allows a single email server to support OAuth2, since there
is only one TIcsRestEmail component and set of secrets.  Also, it does not
automatically clear the last refresh token if the servers account is changed,
you need to do that manually.  Most applications will save the refresh token
instead of the password, and clear that when settings are changed.
...
uses
  ...
  StdCtrls, RichEdit, Mask, ComCtrls, ExtCtrls, Buttons,
  OverbyteIcsIniFiles,
//  OverbyteIcsWSocket,
//  OverbyteIcsSmtpProt,
//  OverbyteIcsWinSock,
...
type
  ...
  // following saved in INI file
    ...
    PrefUserAuth: TComboBox;
    PrefExtBrow: TCheckBox;
    EmlFileName: TEdit;    { V9.2 }
 ...
 // not saved
    ...
    SelectAttachment: TBitBtn;
    OpenDialog: TOpenDialog;
    BoxEML: TGroupBox;
    Label15: TLabel;
    SelectEmlFile: TBitBtn;
    doSendEmlHdr: TButton;
    doSendEmlRecip: TButton;
    doCheckEml: TButton;
    ...
    procedure SelectAttachmentClick(Sender: TObject);
    procedure SelectEmlFileClick(Sender: TObject);
    procedure doSendEmlHdrClick(Sender: TObject);
    procedure doCheckEmlClick(Sender: TObject);
    procedure doSendEmlRecipClick(Sender: TObject);
  private
...
var
    FriendlyName: string ;
begin
    result := IcsParseEmail (DoubleAddr, FriendlyName) ;   // V9.3
end ;
...
begin
  ...
  WriteString (SectionData, 'PrefUserAuth_Text', PrefUserAuth.Text) ;
  if PrefExtBrow.Checked then temp := 'True' else temp := 'False' ; WriteString (SectionData, 'PrefExtBrow_Checked', temp) ;
  WriteString (SectionData, 'EmlFileName_Text', EmlFileName.Text) ;   { V9.2 }
...
// get old settings
  ...
  PrefUserAuth.Text := ReadString (SectionData, 'PrefUserAuth_Text', PrefUserAuth.Text) ;
  if ReadString (SectionData, 'PrefExtBrow_Checked', 'False') = 'True' then PrefExtBrow.Checked := true else PrefExtBrow.Checked := false ;
  EmlFileName.Text := ReadString (SectionData, 'EmlFileName_Text', EmlFileName.Text) ;   { V9.2 }
    ...
    if PrefUserAuth.Text = '' then
        PrefUserAuth.Text := OAuthMsUserAuthDef;           { V8.70 }
end;
...
procedure TDemoForm.SelectAttachmentClick(Sender: TObject);
begin
    OpenDialog.InitialDir := ExtractFileDir(FileAttachment.Text) ;
    if OpenDialog.Execute then
        FileAttachment.Text := OpenDialog.FileName;
end;

procedure TDemoForm.SelectEmlFileClick(Sender: TObject);
begin
    OpenDialog.InitialDir := ExtractFileDir(EmlFileName.Text) ;
    if OpenDialog.Execute then
        EmlFileName.Text := OpenDialog.FileName;
end;
...
begin
    ...
    begin
        ...
        try
            ...
            IcsMailQueue1.DnsServers.Add ('1.1.1.1') ; // Cloudfare  V8.71
         ...
         // add multiple email servers, not necessary if using specific or MX domain lookup
            ...
            begin
                ...
                begin
                    ...
                    if AuthType in [smtpAuthXOAuth2,  smtpAuthOAuthBearer] then begin
                        TokenFlag := True;                                   { V8.65 }
                        IcsRestEmail1.AccountHint := PrefEmailAuthUser1.Text;    { V8.71 }
                    end;
            ...
            begin
                ...
                begin
                    ...
                    if AuthType in [smtpAuthXOAuth2,  smtpAuthOAuthBearer] then begin
                        TokenFlag := True;                                      { V8.65 }
                        IcsRestEmail1.AccountHint := PrefEmailAuthUser2.Text;    { V8.71 }
                    end;
        ...
        { warning, may need different OAuth2 for differenmt SMTP servers }
            ...
            begin
                ...
                IcsRestEmail1.MsUserAuth := PrefUserAuth.Text;       { V8.70 }
                if PrefExtBrow.Checked then                          { V8.71 }
                    IcsRestEmail1.OAAuthType := OAuthTypeWeb
                else
                    IcsRestEmail1.OAAuthType := OAuthTypeEmbed;
...
begin
    ...
    // queue message details
        ...
        IcsMailQueue1.QuHtmlSmtp.Allow8bitChars := false ;    { V8.70 was true which prevents encoding of headers }
        IcsMailQueue1.QuHtmlSmtp.ConvertToCharset := true ;   { V8.70 ignored for Unicode compilers, always converts }
        IcsMailQueue1.QuHtmlSmtp.CharSet := 'utf-8' ;         { V8.70 so full unicode is supported }
        IcsMailQueue1.QuHtmlSmtp.FoldHeaders := True;         { V8.70 }
...
procedure TDemoForm.IcsRestEmail1OAEmbed(Sender: TObject; const URL: string; var Success: Boolean);       { V8.71 }
begin
...
procedure TDemoForm.doCheckEmlClick(Sender: TObject);
var
    MailQuItem: TMailQuItem ;
begin
    if IcsMailQueue1.CheckEML(EmlFileName.Text, MailQuItem) then begin
        MailLogAdd ('Mail From: ' + MailQuItem.XSender);
        MailLogAdd ('Mail To: ' + MailQuItem.XReceivers);
        MailLogAdd ('Mail Subject: ' + MailQuItem.Subject);
    end;
end;

procedure TDemoForm.doSendEmlHdrClick(Sender: TObject);
var
...
begin
    ...
    try // finally
        LabelProg.Caption := '' ;
        doSendEmlHdr.Enabled := false ;
        AbortFlag := false ;
        DiagForm.Visible := true ;

    // stop queue if running so new folder can be set, then start queue if not running
        if IcsMailQueue1.MailQuDir <> PrefMailQuFolder.Text then
            StopQueueMail;
        StartQueueMail;
        if NOT IcsMailQueue1.Active then
            exit ;
        ...
        MailLog.SelStart := 999999999 ;
        try
            LabelCount.Caption :=  'Processing 1 of 1 with 0 Errors' ;
            Refresh ;
         // errcode := 0 ;
            if IcsMailQueue1.SmtpMethod = MailSmtpSpecific then
               item := IcsMailQueue1.QueueEml (EmlFileName.Text, '', PrefEmailSmtp1.Text, PrefEmailSmtp2.Text, '')  // up to three specific mail servers
            else
               item := IcsMailQueue1.QueueEml (EmlFileName.Text, '');
            if item = 0 then
            begin
                info := 'Failed to Queue Mail - ' + IcsMailQueue1.QuHtmlSmtp.ErrorMessage ;
             // errcode := atoi (Copy (IcsMailQueue1.QuHtmlSmtp.ErrorMessage, 1, 3)) ;
        ...
        doSendEmlHdr.Enabled := true ;

    end;end;

procedure TDemoForm.doSendEmlRecipClick(Sender: TObject);
var
    index, donenr, errcode, item, errnr: integer ;
    recip, info: string ;
begin
    ...
    // stop queue if running so new folder can be set, then start queue if not running
        ...
        begin
            ...
            try
                ...
                errcode := 0 ;
                if IcsMailQueue1.SmtpMethod = MailSmtpSpecific then
                   item := IcsMailQueue1.QueueEml (EmlFileName.Text, ExtractEmail (recip), PrefEmailSmtp1.Text, PrefEmailSmtp2.Text, '')  // up to three specific mail servers
                else
                   item := IcsMailQueue1.QueueEml (EmlFileName.Text, ExtractEmail (recip));
                if item = 0 then
                begin
                    info := 'Failed to Queue Mail - ' + IcsMailQueue1.QuHtmlSmtp.ErrorMessage ;
                    errcode := atoi (Copy (IcsMailQueue1.QuHtmlSmtp.ErrorMessage, 1, 3)) ;
