...
Nov 25, 2020 V8.65 - Added XOAuth2 and OAuthBearer authentication support using
               ...
               Note you have to specify ClientId and ClientPassword later
...
Aug 5, 2022  V8.70 - Added header character set and body line wrapping and
                encoding from non-SSL sample.
             Note the UseMailMessageCheckBox means you pass the entire
                body in MailMessage property instead of using the OnGetData
                event to send one line at a time, this will also word wrap
                long lines and encode body content.
Dec 19, 2022 V8.71 Support OAuth authentication OAuthTypeEmbed (embedded browser)
                using new TOAuthBrowser component and TOAuthLoginForm window, that
                uses TEdgeBrowser or TWebBrowser to display web page without
                needing external browser or web server, traps redirect request
                to get authentication code, Edge is Delphi 10.4 or later only.
...
Sep 24, 2024 V9.3  Using OverbyteIcsTypes for consolidated types and constants, allowing
                     other import units to be removed.
                   Using Segoe UI font which scale properly.
                   Cleaned up form, display more lines in log.
...
//OverbyteIcsSslHttpRest,
  ...
  OverbyteIcsSslBase,      { V8.70 }
  OverbyteIcsTypes;        { V9.3 consolidated types and constants }

const
  SslSmtpTestVersion    = 903;
  CopyRight : String    = ' SslMailSnd (c) 1997-2024 F. Piette V9.3 ';
...
type
  TSslSmtpTestForm = class(TForm)
    ...
    OAuth2ExtBrow: TCheckBox;
    Label22: TLabel;
    OAuth2UserAuth: TComboBox;
...
const
    ...
    KeyOAuth2ExtBrow    = 'OAuth2ExtBrow';
    KeyOAuth2UserAuth   = 'OAuth2UserAuth';
...
begin
    ...
    try
        if DisplayMemo.Lines.Count > 10000 then begin   { V9.3 was 200 lines }
            { We preserve only 200 lines }
            while DisplayMemo.Lines.Count > 10000 do
...
begin
    if not FInitialized then begin
        ...
        OAuth2UserAuth.Text :=  IniFile.ReadString(SectionData, KeyOAuth2UserAuth,  '');   { V8.71 }
        OAuth2ExtBrow.Checked := IniFile.ReadBool(SectionData, KeyOAuth2ExtBrow, False);   { V8.71 }
      ...
      { V8.70 character set stuff copied from non-SSL sample }
        ...
        Allow8BitCheckBox.Checked := IniFile.ReadBool(SectionData, KeyAllow8bit, False);
        FoldHeadersCheckBox.Checked := IniFile.ReadBool(SectionData, KeyFoldHeaders, True);
        ...
        IcsCharsetComboBox1.Charset := IniFile.ReadString(SectionData, KeyCharset, SslSmtpClient.CharSet);
        DefEncodingComboBox.ItemIndex := IniFile.ReadInteger(SectionData, KeyDefTransEnc, 0);
        if OAuth2UserAuth.Text = '' then
            OAuth2UserAuth.Text := OAuthMsUserAuthDef;                                   { V8.71 }
...
begin
    ...
    IniFile.WriteString(SectionData, KeyOAuth2UserAuth,   OAuth2UserAuth.Text);        { V8.71 }
    IniFile.WriteBool(SectionData, KeyOAuth2ExtBrow,      OAuth2ExtBrow.Checked);      { V8.71 }
    ...
    SaveStringsToIniFile(IniFile, SectionFileAttach,      KeyFileAttach, FileAttachMemo.Lines);
    SaveStringsToIniFile(IniFile, SectionMsgMemo,         KeyMsgMemo, MsgMemo.Lines);
...
// TEMP STUFF !!!
//    IcsRestEmail1.RestEmailType := RestEmailMSSmtp;
//    IcsRestEmail1.ClientId := 'xxxxxxxxx';
//    IcsRestEmail1.ClientSecret := 'xxxxxxxxxxx';
    ...
    IcsRestEmail1.AccountHint := UsernameEdit.Text;        { V8.71 }
    IcsRestEmail1.MsUserAuth := OAuth2UserAuth.Text;       { V8.71 }
    if OAuth2ExtBrow.Checked then                          { V8.71 }
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
{ change Authentication Type, clear OAuth2 token }
procedure TSslSmtpTestForm.AuthComboBoxChange(Sender: TObject);
begin
    if NOT (TSmtpAuthType (AuthComboBox.ItemIndex) in [smtpAuthXOAuth2, smtpAuthOAuthBearer]) then begin
        if Length(PasswordEdit.Text) > 32 then PasswordEdit.Text := '';  // clear old token
    end;
end;
...
begin
  ...
  { V8.70 Assign property MailMessage and unassign OnGetData if you need }
  { automatic encoding and line wrapping                           }
    if UseMailMessageCheckBox.Checked then begin
        SslSmtpClient.MailMessage        := MsgMemo.Lines;
        SslSmtpClient.OnGetData          := nil;
        SslSmtpClient.WrapMsgMaxLineLen  := StrToIntDef(WrapAtEdit.Text, 76);
        SslSmtpClient.WrapMessageText    := WrapTextCheckBox.Checked;
    end
    else begin
        SslSmtpClient.OnGetData   := SslSmtpClientGetData;
        SslSmtpClient.MailMessage.Clear;
    end;

 { V8.70 character set stuff copied from non-SSL sample }
    SslSmtpClient.CharSet            := IcsCharsetComboBox1.Charset;        { generally UFF-8 }
    SslSmtpClient.ConvertToCharset   := ConvertToCharsetCheckBox.Checked;   { always converts for Unicode compilers }
    SslSmtpClient.DefaultEncoding    := TSmtpDefaultEncoding(DefEncodingComboBox.ItemIndex);
    SslSmtpClient.Allow8bitChars     := Allow8BitCheckBox.Checked;          { must be off to allow inline encoding }
    SslSmtpClient.FoldHeaders        := FoldHeadersCheckBox.Checked;
...
begin
 ...
 { V8.70 character set stuff copied from non-SSL sample }
    SslSmtpClient.CharSet            := IcsCharsetComboBox1.Charset;
    SslSmtpClient.ConvertToCharset   := ConvertToCharsetCheckBox.Checked;
    SslSmtpClient.DefaultEncoding    := TSmtpDefaultEncoding(DefEncodingComboBox.ItemIndex);
    SslSmtpClient.Allow8bitChars     := Allow8BitCheckBox.Checked;
    SslSmtpClient.FoldHeaders        := FoldHeadersCheckBox.Checked;
...
begin
    if SslSmtpClient.Connected then begin
        MessageBeep(MB_OK);
        Display('All-In-One demo started in connected state.');
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
{ called by TIcsRestEmail to display the embedded browser window }
procedure TSslSmtpTestForm.IcsRestEmail1OAEmbed(Sender: TObject; const URL: string; var Success: Boolean);    { V8.71 }
begin
