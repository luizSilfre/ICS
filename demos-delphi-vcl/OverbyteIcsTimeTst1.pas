...
type
  TTimeDemoForm = class(TForm)
    ...
    procedure doTestConvClick(Sender: TObject);
  private
  end;
...
procedure TTimeDemoForm.doTestConvClick(Sender: TObject);
var
    NtpTime: TNtpTime;
begin
    NtpTime := IcsGetUTCNtpTime;
    DisplayMemo.Lines.Add ('TNtpTime: ' + IntToStr(Int64(NtpTime.Seconds)) + ' seconds, ' + IntToStr(Int64(NtpTime.FracSecs)) + ' fractional seconds');
    DisplayMemo.Lines.Add ('Now converted back to TDateTime: ' +  IcsTimeToZStr(IcsNtp2TDateTime (NtpTime)));
end;
...
begin
    ...
    begin
        if DateTime = 0 then begin
            DisplayMemo.Lines.Add ('Unable to Get Network Time, Server: ' + ServerAddr + ' [' + IpAddress + '] - ' + LastProgress);
            doCorrect.Enabled := False;
        end
        else
        begin
            DisplayMemo.Lines.Add ('Response from Server: ' + ServerAddr + ' [' + IpAddress + '], Attempts ' + IntToStr (Attempts)) ;
            DisplayMemo.Lines.Add ('UTC/GMT ' + IcsDateTimeZZtoStr (DTTransmit));
            DisplayMemo.Lines.Add ('Difference: ' + SecsDiffDisp + ' secs, Roundtrip: ' + IcsIntToCStr (MsecsRoundtrip) + 'ms');
                  ...
                  // correct PC clock by time difference, not absolute  due to roundtrip delays
                    if doCorrect.Enabled then begin
                        ...
                        DisplayMemo.Lines.Add ('!!! Changing system time from UTC: ' +
                                                          IcsDateTimeZZtoStr (OldUtcDT) + ' to ' + IcsDateTimeZZtoStr (NewUtcDT));
