...
The FTP server will also automatically create self signed certificates to allow
it to start, if certificate files are missing.
...
18 Aug 2022 V8.70 Added TIcsRestEmail to support OAuth2 authentication for GMail when
                    ...
                    client account.
                  Fixed UnEscapeStr for unicode compilers.
                  Added IcsLogger for SSL debugging.
Feb 14, 2023 V8.71 Removed MadExcept
...
const
    SrvCopyRight : String = ' OverbyteIcsSslMultiFtpServ (c) 2024 Francois Piette V9.3 ';
    MaxWinChars = 800000;
...
type
  TFtpServerForm = class(TForm)
    ...
    DiagsSslLow: TCheckBox;
    IcsLogger1: TIcsLogger;
    IcsIpStrmLog1: TIcsIpStrmLog;
    IcsIpStrmLog2: TIcsIpStrmLog;
    ...
    procedure IcsLogger1IcsLogEvent(Sender: TObject; LogOption: TLogOption; const Msg: string);
  private
    FIniFileName : String;
    FFinalized   : Boolean;
...
begin
   ...
   if DiagsSslLow.Checked then         { V8.70 }
      IcsIpStrmLog1.SendLogLine(AnsiString(Msg));
...
//        TempStrings.Clear;    // !!! TEMP CRASH
 ...
 // house keeping every five minutes
    if IcsTestTrgTick64(HouseKeepingTrg) then begin  { V8.71 was not 64 }
...
//    OverbyteIcsWSocket.LoadSsl;    { V9.1 gone }
   ...
   { try and open INI file in C:\Users\(me)\AppData\Local\ICS, if missing copy default from sample directory  }
    try
        if NOT FileExists(FIniFileName) then begin
            FName := FAccountPath + ChangeFileExt(ExtractFileName(ParamStr(0)), '.ini');  { V9.1 lower directory }
    ...
    Display('FTP Accounts Path: ' + FAccountPath);  { V9.1 }
    DiagLogBuffer.FlushFile(True);
...
var
    ...
    begin
        ...
        Move (S2 [1], result [TxLen + 1], (Length (S2) * SizeOf(Char)));   { V8.70 unicode fix }
        inc (TxLen, Length (S2)) ;
    end ;
...
begin
    ...
    begin
       ...
       (*     else if (Ch = 'i') then    { V8.70 }
            begin
                BuildTxString2 (FormatIpAddrPort (FormatIpAddr (Server.Addr), Server.Port)) ;
            end     *)
            else if (Ch = 'h') then    { V8.70 }
            begin
                BuildTxString2 (SslFtpServer1.IcsHosts[0].HostNames[0]) ;
            end
            else if (Ch = 'v') then   { V8.70 }
            begin
                BuildTxString2 (SrvCopyRight) ;
            end
            else if (Ch = 's') then
            begin
...
begin
    try
       // stream log using UDP to localhost  V8.70
       if DiagsSslLow.checked then begin
            IcsIpStrmLog1.StartLogging;
       end;

        if DiagsSslLow.Checked then begin                                                                      { V8.70 }
            IcsLogger1.LogOptions := [loDestEvent] + LogAllOptInfo + [loAddStamp];
        end;
...
//        if (SslFtpServer1.RootCA = '') or (NOT FileExists(SslFtpServer1.RootCA)) then  { V9.1 gone }
...
begin
    ...
    if DiagsSslLow.checked then
            IcsIpStrmLog1.StopLogging;
...
begin
  ...
  { get INI file for default accounts, may be changed if HOST command used }
    Client.AccountIniName := FtpServerForm.FAccountPath + 'ftpaccounts-default.ini';
...
{ HOST might be ftp.domain.com or [123.123.123.123]   }
    fname := FAccountPath + 'ftpaccounts-' + Lowercase (Host) + '.ini';
...
{ * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TFtpServerForm.IcsLogger1IcsLogEvent(Sender: TObject; LogOption: TLogOption; const Msg: string);                          { V8.70 }
begin
    Display(Msg);
end;
...
{ * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TFtpServerForm.IcsMailQueueOATokenEvent(ServNr: Integer; var Token, TokAccount: string; var TokExpireDT: TDateTime);      { V8.70 }
begin
...
{ * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TFtpServerForm.IcsRestEmailEmailNewToken(Sender: TObject);                                                                 { V8.70 }
var
    info: String;
begin
// we should save refresh token since only get it after a login window
    if (IcsRestEmail.RefrToken <> '') and (IcsRestEmail.RefrToken <> OldRefrToken) then begin
        ...
        Display(info);
        if (AdminEmailTo <> '') then
            SendAdminEmail (AdminEmailTo, 'ICS Multi FTP Server New OAuth2 Refresh Token',
            'FTP server New OAuth2 Refresh Token.' + IcsCRLF + IcsCRLF + info);
    end;
end;


{ * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TFtpServerForm.IcsRestEmailEmailProg(Sender: TObject; LogOption: TLogOption; const Msg: string);                           { V8.70 }
begin
    Display(Msg);
end;
...
{ * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TFtpServerForm.IcsRestEmailOAAuthUrl(Sender: TObject; const URL: string);                                                  { V8.70 }
begin
//
end;
...
begin
 ...
 // force certiificate check to load new ones
    ...
    Display('FTP server ordered new SSL cerrtificate.' + IcsCRLF +
                                      IcsSslX509Certs.GetOrderResult);
    if (AdminEmailTo <> '') then
        SendAdminEmail (AdminEmailTo, 'ICS Multi FTP Server SSL Certificate Order Completed',
        'FTP server ordered new SSL cerrtificate.' + IcsCRLF + IcsCRLF +
...
procedure TFtpServerForm.IcsSslX509CertsOAuthAuthUrl(Sender: TObject;
  const URL: string);
begin
   Display('FTP server demo needs OAuth authenfication for new certificate, ' +
                'Browse to this URL: ' + URL +  ', From PC: ' + IcsGetCompName) ;
    if (AdminEmailTo <> '') then
        SendAdminEmail (AdminEmailTo, 'ICS Multi FTP Server needs OAuth authenfication for new certificate',
