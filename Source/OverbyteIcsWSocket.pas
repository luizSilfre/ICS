...
{$ELSE FMX} // VCL
  ...
  OverbyteIcsSslBase,  { V9.1 TX509Base, TX509List, etc }
{$ENDIF FMX}
  OverbyteIcsNtlmMsgs,
  OverbyteIcsWSockBuf,
  OverbyteIcsMimeUtils,
  OverbyteIcsTypes,
  OverbyteIcsUrl                                    { V8.66 }
...
const
  WSocketVersion            = 904;
  CopyRight    : String     = ' TWSocket (c) 1996-2025 Francois Piette V9.4 ';
...
{$ENDIF}
    ...
    if FState <> wsConnected then begin
        if (IcsHiWord(msg.LParam) = 0) then { V9.1 don't set connected if it failed }
            ChangeState(wsConnected);
  ...
  {$ENDIF}
    ...
    if FType = SOCK_DGRAM then begin
      ...
      {$ENDIF}
        ...
        iStatus := WSocket_Synchronized_Connect(FHSocket, PSockAddrIn(@Fsin)^, SizeOfAddr(Fsin));
        if iStatus = 0 then begin
...
{$ENDIF}
    ...
    iStatus := WSocket_Synchronized_bind(FHSocket, PSockAddr(@Fsin)^, SizeOfAddr(Fsin));
    if iStatus = 0 then
        ChangeState(wsBound)
    else begin
        try
               ...
               { V8.39 get pointer to verify parameters, which we may alter in a moment }
                ...
                if (FSslServerName <> '') then begin
                 { V8.64 needs A-Label punycode, not UTF8 }
                    ...
                    if (Ics_SSL_set_tlsext_host_name(FSsl, String(AHost)) = 0) then
