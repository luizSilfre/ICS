...
Jun 25, 2023 V8.71 Changed property LmCompatLevel from public to published so it's
                   ...
                   Above fixes thanks to Jaroslav Kulísek.
...
const
    HttpCliVersion       = 904;
    CopyRight : String   = ' THttpCli (c) 1997-2025 F. Piette V9.4 ';
    ...
    try
     ...
     { V8.64 needs A-Label punycode for Host: not Unicode }
        ...
        if (Method = 'CONNECT') then begin                             // <= 12/29/05 AG
            if FAlpnProtoList.Count > 0 then     { V9.1 add ALPN header }
                Headers.Add('ALPN: ' + String(IcsPercentEncode(AnsiString(FAlpnProtoList.CommaText))));
...
begin
  ...
  { V8.62 set Alpn Protocols in SslCtx., V9.1 now in FCtrlSocket }
    if (FAlpnProtoList.Count > 0) and FCtrlSocket.SslEnable then
