...
Jun 4, 2021 - V8.67 - GetIcsIniFileName now has (Local: Boolean = True) for local
                        user app data (C:\Users\(user)\AppData\Local\ICS), but false
                        is now all users common app data instead, c:\ProgramData\ICS.
                      GetCommonAppDataFolder nows uses IcsGetShellPath in Utils.
...
{$IFDEF MSWINDOWS}
  function GetCommonAppDataFolder(const SubPath: String; Local: Boolean = True): String;  { V8.67 added local }
{$ENDIF}
  function GetIcsIniFileName(Local: Boolean = True): String;  { V8.67 added local }
...
{ V8.67 now using IcsGetShellPath from Utils }
function GetCommonAppDataFolder(const SubPath: String; Local: Boolean = True): String;
var
    CSIDL: Integer;
begin
    if Local then
        CSIDL := CSIDL_LOCAL_APPDATA    // (user)\Local Settings\Application Data
    else
        CSIDL := CSIDL_COMMON_APPDATA;  // All Users\Application Data
    Result := String(IcsGetShellPath(CSIDL));
    if Result = '' then
        Exit;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function GetIcsIniFileName(Local: Boolean = True): String;
begin
  {$IFDEF MSWINDOWS}
    Result := GetCommonAppDataFolder('ICS', Local);
  {$ELSE}
