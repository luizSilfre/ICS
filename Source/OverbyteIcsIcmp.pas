...
const
  IcmpVersion = 904;
  CopyRight : String   = ' TICMP (c) 1997-2025 F. Piette V9.4 ';
  IcmpDLL     = 'icmp.dll';
...
{$ENDIF MSWINDOWS}
{$IFDEF POSIX}
  {$MESSAGE 'TODO ICMP/Ping'}           { V8.65 pending Linux }
{$ENDIF POSIX}
end;
    ...
    // need a DNS lookup of host name, might return IPv4 or IPv6 address
        ...
        try
            ...
            on E: Exception do begin
                FLastError := 100;  { V8.66 should also set error }
                FLastErrStr := { 'Unable to resolve ' + FAddress + ' - ' + } E.Message;  { V8.66 duplicate error }
                exit ;
            end;
        end;
    end;
...
{$IFDEF POSIX}
    Result     := 999;
    FLastError := 999;
    FLastErrStr := 'ICMP/Ping Not Supported Yet';
    Exit;
{$ELSE}
    Result     := 0;
    FLastError := 0;
...
begin
    ...
    if Result > 0 then
        ...
        if FLastError <> 100 then   { V8.66 already set none windows error }
...
begin
    ...
    if (Result = 0) {$IFDEF MSWINDOWS} and (GetLastError = ERROR_IO_PENDING) {$ENDIF MSWINDOWS} then
