...
function IcsJoseCheckJWS(const JWS, OldNonce, HmacSecret: String; var NewPublicKey: PEVP_PKEY; var NewKeyid, Payload: String): TJoseVerify;  { V8.65 }
...
function IcsJoseFindHash(JoseAlg: TJoseAlg): TEvpDigest;
begin
    Result := JoseAlgDigests[JoseAlg];   { V9.4 }
...
begin
    ...
    if ((JoseAlg >= jsigHmac256) and (JoseAlg <= jsigHmac512)) then begin
        ...
        Result := IcsBase64UrlEncodeTB(IcsAsymSignDigestTB(CombinedEn, PrivateKey, HashDigest));
end;
...
begin
    ...
    if ((JoseAlg >= jsigHmac256) and (JoseAlg <= jsigHmac512)) then begin
        ...
        Result := IcsAsymVerifyDigestTB(CombinedEn, SignatureEn, PublicKey, HashDigest);
    end;
end;
...
begin
    ...
    // finally verify signature
       if IcsJoseCheckSigTB(JoseAlg, IcsStringAToTBytes(CombinedEn), IcsBase64UrlDecodeATB(SignatureEn),
                                                                    IcsStringToTBytes(HmacSecret), NewPublicKey) then begin { V9.1 }
            if (JoseAlg >= jsigRsa256) then begin
               if Jwk <> '' then
