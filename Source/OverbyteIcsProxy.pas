...
Jun 20, 2023 V8.71 Ensure inherited destroy called.
                   Using Int64 ticks.
                   Added OnSrvSslHandshakeDone event called when new client connects to
                     proxy server allowing application to check for a client certificate
                     and abort the connection, needs CliCertMethod to be set in IcsHosts
                     to sslCliCertRequire or sslCliCertOptionalso a client certificate
                     is requested.
...
Jan 21, 2024 V9.4  Updated Base64 encoding functions to IcsBase64 functions.
...
const
    ...
    CopyRight : String = ' TIcsHttpProxy (c) 2025 F. Piette V9.4 ';
    DefServerHeader : string = 'Server: ICS-Proxy-V9.4';
...
{ we may receive two or three requests together, then all the responses }
  THttpInfo = record
    ...
    TickReqStart: Int64;                             // when request started
    TickReqSend: Int64;                              // when request was forwarded
    TickWaitResp: Int64;                             // when request finished. wait response
    TickRespStart: Int64;                            // when response started
    TickRespBody: Int64;                             // when response body started
    TickRespSend: Int64;                             // when response forwarding started
    TickRespDone: Int64;                            // when response forwarding finished
    TickRespEnd: Int64;                             // when response all forwarded   { V8.71 all ticks now Int64 }
  end;
...
{$ENDIF}
    ...
    FOnSrvSslHandshakeDone: TSslHandshakeDoneEvent;  { V8.71 }
...
{$ENDIF} // AUTO_X509_CERTS
    ...
    property  OnSrvSslHandshakeDone: TSslHandshakeDoneEvent
                                                    read  FOnSrvSslHandshakeDone
                                                    write FOnSrvSslHandshakeDone;  { V8.71 }
  end;
...
begin
    if Assigned (FProxySource.FonProxyProg) then begin
        dur := IcsDiffSecs64(Self.Counter.ConnectTick, IcsGetTickCount64);   { V8.71 }
        LogEvent('Source ' + IntToStr (dur) + ' - ' + Msg);                  { V8.71 }
    end;
end ;
...
begin
    if Assigned (FProxySource.FonProxyProg) then begin
        dur := IcsDiffSecs64(FTarSocket.Counter.ConnectTick, IcsGetTickCount64);  { V8.71 }
        LogEvent('Target ' + IntToStr (dur) + ' - ' + Msg);                       { V8.71 }
    end;
end ;
...
{$ENDIF}
      ...
      { note this is a soft check, if we don't have a stapled OCSP response from the TLS handshake, we get it from an
        ...
        if (Safe and FProxySource.SslRevocation and PeerCert.IsCertLoaded and (CertChain.Count > 0)) then begin
            ...
            FProxySource.FOcspTarHttp.DebugLevel := OverbyteIcsTypes.DebugSsl;    { V9.3 }
...
{$ENDIF}
    ...
    FCurDate := Trunc(Date);
    FFlushTick := IcsGetTickCount64;    { V8.71 }
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
{ server event, accepting a new connection }
...
begin
    try
        ...
        { V8.71 event to allow client certificate to be checked and connection killed }
            if Assigned(FOnSrvSslHandshakeDone) then begin
                FOnSrvSslHandshakeDone(Sender, ErrCode, PeerCert, Disconnect);
                if Disconnect then begin
                    MyClient.LogSrcEvent('Client Being Disconnected by Command');
                    Exit;
                end;
            end;
...
begin
  ...
  { periodically flush SSL host list so certificates are rechecked }
    Duration := IcsDiffSecs64 (FFlushTick, CurTicks) div 60;     { V8.71 }
    if (Duration > FlushSslCacheMins) then begin
        FFlushTick := IcsGetTickCount64;
    ...
    try
        ...
        try
            for I := Pred (FSourceServer.ClientCount) downto 0 do begin
                ...
                if MyClient.FClosingFlag then
                    Continue ;
                if MyClient.FSessionClosedFlag then
                    Continue;  // Client will close soon
                if MyClient.Counter = Nil then
                    Continue;

                { different length timeouts depending on what's happening }
                Timeout := MyClient.FIdleTimeout;
                if Timeout > 0 then begin
                    Duration := IcsDiffSecs64(MyClient.Counter.LastAliveTick, CurTicks);       { V8.71 }
                    ...
                    if (MyClient.FTarSocket.State = wsConnected) then begin  // see if receiving remote data
                        ...
                        Duration := IcsDiffSecs64(MyClient.FTarSocket.Counter.LastAliveTick, CurTicks);      { V8.71 }
...
begin
  ...
  { processing response, event is triggered in other places  }
    ...
    if (FHttpInfo [FHttpCurrResp].HttpReqState = httpStRespDone) then begin
        FHttpInfo [FHttpCurrResp].TickRespEnd := IcsGetTickCount64;         { V8.71 }
        ...
        if FSize > 0 then begin
            if FSize < (FProxySource as TIcsHttpProxy).FHttpMaxBody then begin
                ...
                if FHandle <> 0 then begin
                    ...
                    FileClose(FHandle);
                    BodyStr := String(AContent); { V8.71 ascii to unicode }
                end;
            end
            else
        ...
        if FPxyReqState = PxyHdrFind then begin
          ...
          { increment request pipeline and keep it, client may send us several requests
            ...
            with FHttpInfo[FHttpCurrReq] do begin
                ...
                TickReqStart := IcsGetTickCount64;    { V8.71 }
                TickReqSend := 0;
                TickWaitResp := 0;
                TickRespStart := 0;
                TickRespBody := 0;
   ...
   { send response header to source client }
    ...
    begin
        ...
        if FHttpInfo [FHttpCurrResp].HttpReqState = httpStRespSend then begin
            ...
            FHttpInfo [FHttpCurrResp].TickRespDone := IcsGetTickCount64;    { V8.71 }
        end;
    end;
...
begin
    ...
    while TRUE do begin
      ...
      { waiting for headers to arrive }
        if FPxyRespState = PxyHdrFind then begin
          ...
          { update pipeline responses }
            ...
            FHttpInfo [FHttpCurrResp].TickRespStart := IcsGetTickCount64;    { V8.71 }
      ...
      { no body, just send header, nothing more }
            ...
            SendRespHdr;
            FHttpInfo [FHttpCurrResp].HttpReqState := httpStRespSend;
            FHttpInfo [FHttpCurrResp].TickRespSend := IcsGetTickCount64;    { V8.71 }
            LoopCounter := 0;
            DoneResponse;
            if (FProxySource.DebugLevel >= DebugHttpHdr) then
