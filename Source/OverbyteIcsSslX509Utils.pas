...
Legal issues: Copyright (C) 2007-2024 by François PIETTE
...
begin
    if Source is TSslCertTools then begin
        ...
        State           := LSource.State;
        Locality        := LSource.Locality;
        Organization    := LSource.Organization;
        OrgUnit         := LSource.OrgUnit;
        Descr           := LSource.Descr;
        Email           := LSource.Email;
        CommonName      := LSource.CommonName;
        AltIpList.Assign(LSource.AltIpList);
        AltDNSList.Assign(LSource.AltDNSList);
        AltEmailList.Assign(LSource.AltEmailList);
        AltIssuer       := LSource.AltIssuer;
        CRLDistPoint    := LSource.CRLDistPoint;
        AuthInfoAcc     := LSource.AuthInfoAcc;
        AcmeIdentifier  := LSource.AcmeIdentifier;
        BasicIsCA       := LSource.BasicIsCA;
        BasicPathLen    := LSource.BasicPathLen;
        KeyCertSign     := LSource.KeyCertSign;
        KeyCRLSign      := LSource.KeyCRLSign;
        KeyDigiSign     := LSource.KeyDigiSign;
        KeyDataEnc      := LSource.KeyDataEnc;
        KeyKeyEnc       := LSource.KeyKeyEnc;
        KeyKeyAgree     := LSource.KeyKeyAgree;
        KeyNonRepud     := LSource.KeyNonRepud;
        KeyExtClient    := LSource.KeyExtClient;
        KeyExtServer    := LSource.KeyExtServer;
        KeyExtEmail     := LSource.KeyExtEmail;
        KeyExtCode      := LSource.KeyExtCode;
        ExpireDays      := LSource.ExpireDays;
        SerialNum       := LSource.SerialNum;
        AddComments     := LSource.AddComments;
        PrivKeyType     := LSource.PrivKeyType;
        PrivKeyCipher   := LSource.PrivKeyCipher;
        CertDigest      := LSource.CertDigest;
        CADBFile        := LSource.CADBFile;
        OnKeyProgress   := LSource.OnKeyProgress;
    end;
