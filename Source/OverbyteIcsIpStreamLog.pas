...
Oct 11, 2022 - V8.70  Added UseUtf8 flag that converts received lines from UTF-8 to
                        Unicode with unicode compilers (as String) and converts sent
                        data to UTF-8.
                      Changed FRxBuffer to Tbytes, use ReceiveTB methods with TBytes.
                      Added SendLogLine(Line: UnicodeString), send as UTF-8.
                      Added SendLogLine(Line: TBytes), send raw.
                      Break receive loop if continually receiving non-acceptable characters.
...
const
    CopyRight : String = ' TIcsIpStrmLog (c) 2025 V9.4 ';
            ...
            try
                ...
                begin
                    ...
                    begin
                        ...
                        SocRemAddr.sin_addr.S_addr := WSocket_inet_addr (RemIP) ;    { V8.70 }
...
begin
    ...
    xmitline := Line;
    len := Length (xmitline) ;
    if FAddCRLF and (NOT (FUdpNoCRLF and (FLogProtocol in [logprotUdpClient, logprotUdpServer]))) then
    begin
        SetLength (xmitline, len + 2);
        xmitline[len] := IcsbCR;
        len := len + 1;
        xmitline[len] := IcsbLF;
        len := len + 1;
...
{$ELSE}
                            FonLogRecvEvent (Self, Socnr, UTF8ToStringA(LastLine))
{$ENDIF}
                        else
