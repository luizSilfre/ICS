...
Dec 29, 2021 V8.68 Support If-Range, If-Match and If-None-Match request headers
                     for conditional pages using Etag or last modified date, send
                     304 not modified for matches.  Helps with caching.
                   Add ETag header to responses in AnswerStream and AnswerPage when
                     we can create one from a file modification date and size (base64
                     CRC32), or if the EntityTag property is specified in the
                     onGetDocument event before using hgSendDoc or hgSendStream perhaps
                     a CRC32 of the entire content from a cache.
                   ...
                   WARNING - applications that add Date: or Etag: to CustomHeaders
                     or to an AnswerStream should remove them since they are added
                     automatically.
May 18, 2022 V8.69 Added OCSP (Online Certificate Status Protocol) support using the TOcspHttp
                   ...
                   Added new authentication type atDigestSha2 with a SHA256 hash instead of
                     MD5 with atDigest only supported with USE_SSL. Note: Mozilla Firefox
                     supports Digest SHA-256, Chrome and Edge do not.
                   Added new client connection OnHttpRespHdr event to allow response headers
                     to be logged, previously only request headers could be logged.  Cleaned
                     up AnswerStream functions so separate lines don't go into send buffer.
Oct 19, 2022 V8.70 Simplified ZLIB support to allow use of System.ZLib.
Jan 26, 2023 V8.71 Fixed a Win64 issue reading streams in THttpRangeStream.Read, thanks
                     to Jaroslav Kulísek.
...
Sept 16, 2024 V9.3 Avoid repeated redirection for virtual default page /, was adding /// etc,
                     bug in V9.2.
...
//    OverbyteIcsWinsock,
    ...
    OverbyteIcsCRC,        { V8.68 }
    OverbyteIcsTicks64,    { V8.71 }
    OverbyteIcsHtmlUtils,  { V9.1 }
    OverbyteIcsUrl;        { V9.1 }

const
    THttpServerVersion = 904;
    CopyRight : String = ' THttpServer (c) 1999-2024 F. Piette V9.4 ';
    DefServerHeader : string = 'Server: ICS-HttpServer-V9.4';   { V8.09 }
...
type
    ...
    THttpRespHdrEvent= procedure (Sender          : TObject;                 { V8.69 }
                                  Client          : TObject;
    ...
    THttpOption          = (hoAllowDirList, hoAllowOutsideRoot, hoContentEncoding, { V7.20 }
                            ...
                            hoIgnoreIfModSince, hoAddMissPath);              { V8.11, V9.2 }
...
{$IFNDEF NO_AUTHENTICATION_SUPPORT}
    TAuthenticationType     = (atNone, atBasic
                              ...
                              {$IFNDEF NO_DIGEST_AUTH}, atDigestSha2 {$ENDIF});    { V8.69 }
...
{$ENDIF}
    protected
        ...
        FRequestStartTick      : Int64;                { V8.12 }  { V8.71 }
        ...
        FRequestIfRange        : String;               { V8.68 }
        FRequestIfMatch        : String;               { V8.68 }
...
{$ENDIF}
        ...
        procedure BuildETag(ContentSize: Int64); virtual;                 { V8.68 }
    public
...
{$ENDIF}
        ...
        property RequestStartTick      : Int64       read  FRequestStartTick;  { V8.12 }   { V8.71 }
        property RequestProtocol       : String      read  FRequestProtocol;   { V8.49 }
        property RequestIfRange        : String      read  FRequestIfRange;    { V8.68 }
        property RequestIfMatch        : String      read  FRequestIfMatch;    { V8.68 }
        property RequestIfNoneMatch    : String      read  FRequestIfNoneMatch;{ V8.68 }
    published
...
{$IFNDEF NO_AUTHENTICATION_SUPPORT}
    HttpAuthTypeNames : array [TAuthenticationType] of String =
         ('None', 'Basic' {$IFNDEF NO_DIGEST_AUTH}, 'Digest-MD5' {$ENDIF}
          {$IFDEF USE_NTLM_AUTH}, 'NTLM' {$ENDIF}
          {$IFNDEF NO_DIGEST_AUTH}, 'Digest-Sha256' {$ENDIF});      { V8.69 }
{$ENDIF}
...
begin
    ...
    CreateSocket;
{$IFDEF NO_ADV_MT}
//    FWSocketServer.Name := ClassName + '_SrvSocket' + IntToStr(WSocketGCount);      { V8.69 not used }
{$ELSE}
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure THttpServer.TriggerRespHdr
    (Client : TObject;
     const Header    : string);                                     { V8.69 }
begin
    if Assigned(FonHttpRespHdr) then
        FOnHttpRespHdr(Self, Client, Header);
end;
...
var
    ...
    I        : Integer;
    Cli      : THttpConnection;
    Timeout  : Integer; { V8.71 }
begin
    if not FHeartBeatBusy then  { Avoid reentrance }
    try
        FHeartBeatBusy := TRUE;
        CurTicks := IcsGetTickCount64;
        for I := ClientCount - 1 downto 0 do begin
           ...
           if (Timeout > 0) and  (IcsDiffSecs64(Cli.Counter.LastAliveTick, CurTicks) > Timeout) then  { V8.71 }
...
{ V8.71 Ensure inherited destroy called }
    try
        if Assigned(FRequestHeader) then
        ...
        if Assigned(FRequestRangeValues) then
            FreeAndNil(FRequestRangeValues);
{$IFDEF USE_NTLM_AUTH}
...
{$IFDEF USE_SSL}   { new version using OpenSSL functions }
    AuthDigestCalcHA1Ex(FindAlgoDigest(FAuthDigestAlg),
                      FAuthDigestAlg, AnsiString(FAuthUserName),
                      AnsiString(FAuthDigestRealm), AnsiString(Password),
                      AnsiString(FAuthDigestNonce), AnsiString(FAuthDigestCnonce),
                      SessKey);

    AuthDigestGetBodyHashEx(FindAlgoDigest(FAuthDigestAlg), FAuthDigestBody, HEntity);
    AuthDigestCalcResponseEx(FindAlgoDigest(FAuthDigestAlg),
                           SessKey, AnsiString(FAuthDigestNonce),
                           AnsiString(FAuthDigestNc),
                           AnsiString(FAuthDigestCnonce),
                           AnsiString(FAuthDigestQop), AnsiString(FMethod),
                           AnsiString(FAuthDigestUri), HEntity, MyResponse);
...
{$ENDIF}
{$IFNDEF NO_DIGEST_AUTH}
    else if LowerCase(Copy(FRequestAuth, 1, 7)) = 'digest ' then begin
        if (Pos('=sha-256', LowerCase(FRequestAuth)) > 0) then        { V8.69 algorithm=SHA-256 }
            Result := atDigestSha2
        else
            Result := atDigest      { algorithm=MD5, default }
    end
{$ENDIF}
...
{$IFNDEF NO_DIGEST_AUTH}
    else if AuthType in [atDigest, atDigestSha2] then begin     { V8.69 added Sha2 }
        FAuthDigestBody := '';
...
begin
    ...
    if FState = hcRequest then begin
        ...
        FRequestStartTick      := IcsGetTickCount64;  { V8.12 }  { V8.71 }
        FRequestIfRange        := '';               { V8.68 }
        FRequestIfMatch        := '';               { V8.68 }
        FRequestIfNoneMatch    := '';               { V8.68 }
        FEntityTag             := '';               { V8.68 }
        FDataSent              := 0;      {TURCAN}
        FDocSize               := 0;      {TURCAN}
{$IFNDEF NO_AUTHENTICATION_SUPPORT}
...
{$ENDIF}
    ...
    if I > 0 then begin
        try
            ...
            else if StrLIComp(@FRcvdLine[1], 'If-Range:', 9) = 0 then begin   { V8.68 conditional range request with Etag or LastModDate }
                FRequestIfRange := Copy(FRcvdLine, I, Length(FRcvdLine));
            end
            else if StrLIComp(@FRcvdLine[1], 'If-Match:', 9) = 0 then begin   { V8.68 conditional request with multiple Etags }
                FRequestIfMatch := Copy(FRcvdLine, I, Length(FRcvdLine));
            end
            else if StrLIComp(@FRcvdLine[1], 'If-None-Match:', 14) = 0 then begin   { V8.68 conditional request with multiple Etags }
                FRequestIfNoneMatch := Copy(FRcvdLine, I, Length(FRcvdLine));
            end;
        except
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
{V8.68 build Entity Tag CRC of time stamp and size, unless application provided
  something better like CRC of content }
procedure THttpConnection.BuildETag(ContentSize: Int64);        { V8.68 }
begin
    if FEntityTag <> '' then Exit;  // user generated it
    if (ContentSize <= 0) or (FLastModified = 0) then Exit;
    FEntityTag := IcsDQUOTE + IcsBase64UrlEncode(
       StrCRC32(RFC3339_DateToStr(FLastModified, False) + IntToStr(ContentSize))) + IcsDQUOTE;
end;
...
var
    ...
    RespHeader      : String;       { V8.69 }
begin
    Flags := hgWillSendMySelf;
    ContEncoderHdr := '';
    FLastModified := LastModified;       { V8.67  }

 { V8.68 see if comparing old entity tag with latest version to skip download, we may
   have been given a comma list of etags }
    if not Assigned(FDocStream) then
        BuildETag(0)
    else
        BuildETag(FDocStream.Size);
    ...
    if (FRequestIfMatch <> '') and (FEntityTag <> '') then begin
        if (Pos(FEntityTag, FRequestIfMatch) = 0) then begin   { no etag found, skip }
            Answer304;
            Exit;
         end;
    end;
  { see if document actually changed, within one second of time stamp, V8.68 RequestIfNoneMatch has precidence }
    if (NOT (hoIgnoreIfModSince in FOptions)) and (FLastModified > 0) and
...
begin
    ...
    if FLastModified <> 0 then
        DateStr := RFC1123_Date(IcsDateTimeToUTC (FLastModified)) + ' GMT';      { V8.68, V8.69 UTC }
    CompleteDocSize := FDocStream.Size;

 { V8.68 see if comparing old entity tag with latest version to skip download, we may
   have been given a comma list of etags }
    BuildETag(CompleteDocSize);
...
{ we send one header for each of the authentication types we support,
  let the browser choose which, easiest last  }
...
begin
    ...
    if FLastModified <> 0 then
        DateStr := RFC1123_Date(IcsDateTimeToUTC (FLastModified)) + ' GMT';      { V8.68, V8.69 UTC }
    CompleteDocSize := IcsGetFileSize(FDocument);  { V8.68 get size before opening stream }
    if CompleteDocSize < 0 then begin              { V8.68 nothing to open }
        Answer404;
        Exit;
    end;
...
//    CompleteDocSize := FDocStream.Size;  { V8.68 got it earlier }
    {ANDREAS Create the virtual 'byte-range-doc-stream', if we are ask for ranges}
    if RequestRangeValues.Valid then begin
        { NewDocStream will now be the owner of FDocStream -> don't free FDocStream }
...
begin
    ...
    if Assigned(OnRespHdr) then
        OnRespHdr(Self, Header);                         { V8.69 header we sent do it can be logged }
end;
...
{$IFDEF WIN64}
                SizeRead := Rec.Stream.Read(Pointer(NativeUInt(@Buffer) + NativeUInt(DataRead))^, ActSize); { V8.71 JK x64 }
{$ELSE}
...
{$IFNDEF NO_DIGEST_AUTH}
    ...
    if atDigestSha2 in Types then
        Result := Result + 'atDigestSha2 ';     { V8.69 }
{$ENDIF}
{$IFDEF USE_NTLM_AUTH}
